...
export default function DeleteGroup({ariaLabel}: DeleteGroupProps) {
        ...
        <div className="bg-white rounded max-h-fit ">
          {groups.length > 0 ? (
            groups.map((group, i) => (
              <div
                key={group.id}
                className="flex items-center justify-between bg-white shadow-md rounded px-8 pt-6 pb-8 gap-8">
                <div>
                  <p>{group.name}</p>
                </div>
                <div>
